...
/**
 * Data corresponding to one partition of a ONEPROXYRDD.
 */
private[sql] case class ONEPROXYPartition(psql: String, idx: Int) extends Partition {
  override def index: Int = idx
}

private[sql] object ONEPROXYRDD extends Logging {

  /**
   * Maps a ONEPROXY type to a Catalyst type.  This function is called only when
   ...
   * schema.
   *
   * @param url - The ONEPROXY url to fetch information from.
  ...
  /**
   * Build and return ONEPROXYRDD from the given information.
   *
   * @param sc - Your SparkContext.
   * @param schema - The Catalyst schema of the underlying database table.
   * @param url - The ONEPROXY url to connect to.
...
/**
 * An RDD representing a table in a database accessed via ONEPROXY.  Both the
 * driver code and the workers must be able to access the database; the driver
 * needs to fetch the schema while the workers need to fetch the data.
 */
private[sql] class ONEPROXYRDD(
    sc: SparkContext,
  ...
  private def getExplainSQL: ArrayBuffer[String] = {
        ...
        val sparkrddSQL = s"EXPLAIN2 SELECT $columnList FROM $fqTable $getWhereClause"
  ...
  override def getPartitions: Array[Partition] = {
    ...
    (0 until sqls.length).map { i =>
        new ONEPROXYPartition(sqls(i),i)
    }.toArray	
  }
  ...
  private val filterWhereClause: String =
    filters.flatMap(ONEPROXYRDD.compileFilter).map(p => s"($p)").mkString(" AND ")
  ...
  override def compute(thePart: Partition, context: TaskContext): Iterator[InternalRow] =
    ...
    val stmt = conn.prepareStatement(part.psql,
      ...
      if (rs.next()) {
        ...
        while (i < conversions.length) {
          ...
          conversions(i) match {
            ...
            // So it is needed to set precision and scale for Decimal based on ONEPROXY metadata.
